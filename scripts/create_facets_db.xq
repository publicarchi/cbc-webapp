...
let $communes :=
  <communes>{
    for $c in fn:distinct-values(db:open('cbc')//commune)
    return <commune text="{$c => fn:normalize-space()}" />
  }</communes>
  
let $regions :=
  <regions>{
    for $c in fn:distinct-values(db:open('cbc')//region)
    return <region text="{$c => fn:normalize-space()}" />
  }</regions>
  
let $departements :=
  <departements>{
    for $c in fn:distinct-values(db:open('cbc')//departement[fn:not(@type)])
    return <departement text="{$c => fn:normalize-space()}" />
  }</departements>  
  
let $departementsAnciens :=
  <departementsAnciens>{
    for $c in fn:distinct-values(db:open('cbc')//departementAncien)
    return <departementAncien text="{$c => fn:normalize-space()}" />
  }</departementsAnciens> 

let $buildingTypes :=
  <buildingTypes>{
    for $c in fn:distinct-values(db:open('cbc')//category[@type = "buildingType"])
    return <buildingType text="{$c => fn:normalize-space()}" />
  }</buildingTypes> 
  
let $buildingCategories :=
  <buildingCategories>{
    for $c in fn:distinct-values(db:open('cbc')//category[@type = "buildingCategory"])
    return <buildingCategory text="{$c => fn:normalize-space()}" />
  }</buildingCategories>
  
let $projectGenres := 
   <projectGenres>{
    for $c in fn:distinct-values(db:open('cbc')//category[@type = "projectGenre"])
    return <projectGenre text="{$c => fn:normalize-space()}" />
  }</projectGenres>
  
let $administrativeObjects := 
   <administrativeObjects>{
    for $c in fn:distinct-values(db:open('cbc')//category[@type = "administrativeObject"])
    return <administrativeObject text="{$c => fn:normalize-space()}" />
  }</administrativeObjects>
  
let $participants :=
 <participants>{
    for $c in db:open('cbc')//persName
    return <participant persName="{$c => fn:normalize-space()}" />
  }</participants>
  
let $reports := 
   <reports>{
    for $c in fn:distinct-values(db:open('cbc')//report/author)
